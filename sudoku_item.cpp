...
void SudokuItem::store_board()
{
    ...
    if (file.open(QIODevice::WriteOnly))
    {
        ...
        for (unsigned p = 0; p < I_Sudoku::pos_num; ++p)
        {
            char buff[4];
            buff[0] = key[p];
...
{
    if (val[0] == I_Sudoku::blank)
    {
        if (m_board[pos] == I_Sudoku::blank)
        {
            return;
        }
        else
    ...
    else if (m_board[pos] == I_Sudoku::blank)
    {
        --m_hidden_count;
    }
...
void SudokuItem::check_board()
{
    ...
    for (uint pos = 0; pos < I_Sudoku::pos_num; ++pos)
    {
...
QString SudokuItem::get_current_game_profile_string()
{
    ...
    return QString::number(hidden_count) + " / " + QString::number(I_Sudoku::pos_num - hidden_count);
}
